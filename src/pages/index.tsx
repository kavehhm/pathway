...
const DIFFERENTIATORS = [
  ...
  },
  {
    ...
    description:
      "We verify .edu emails, approvals, and payment readiness so you only meet tutors who are prepared for your first session.",
  },
];

const TESTIMONIALS = [
  {
    name: "Maya, admitted to UCLA",
    quote:
      "My mentor walked me through the exact essay that got them in. I booked them right after our first session.",
  },
  {
    name: "Jordan, Northwestern sophomore",
    quote:
      "Pathway helped me find a tutor who already took CS 214. I stopped spinning my wheels and nailed the midterm.",
  },
  {
    name: "Priya, Brown neuroscience",
    quote:
      "The first session free policy made it easy to try. I now have a go-to mentor for every tough week.",
  },
];
                ...
                <div className="relative rounded-[2.5rem] border border-white/60 bg-white/70 p-8 shadow-2xl backdrop-blur">
                  <div className="flex flex-col gap-6">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-slate-500">Current mentors</span>
                      <Link
                        href="/tutors"
                    ...
                    <div className="space-y-5">
                      {(approvedTutors.data ?? []).slice(0, 3).map((mentor) => {
                        ...
                        return (
                          <div
                            key={mentor.id}
                            className="flex items-center justify-between rounded-2xl border border-slate-100 bg-white/90 px-4 py-3 shadow-sm transition hover:-translate-y-1 hover:shadow-md"
                          >
                            <div>
                              ...
                              <span className="mt-2 inline-flex rounded-full bg-violet-100 px-2 py-0.5 text-[11px] font-medium text-violet-700">
                                {badge}
                              </span>
                            </div>
